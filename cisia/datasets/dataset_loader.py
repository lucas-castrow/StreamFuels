...
class DatasetLoader:
    ...
    def download_anp(download_path='./', transaction_type=None, location_type=None, fuel_type=None):
        ...
        """
        if transaction_type == "sales" and location_type == "city" and fuel_type is None:
            raise ValueError("fuel_type is required when transaction_type is 'sales' and location_type is 'city'. Please provide a valid fuel_type.")
        
        filename = download_anp_data(transaction_type=transaction_type, location_type=location_type, fuel_type=fuel_type)
        
        if location_type == "state":
            tsf_path = processar_dpee_mes_estado(download_path, filename=filename)
        elif location_type == "city":
            tsf_path = processar_derivados_municipio_ano(download_path=download_path,filename=filename, fuel_type=fuel_type)
